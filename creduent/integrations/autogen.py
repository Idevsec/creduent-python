...
if HAS_AUTOGEN:
    ...
    class CreduentConversableAgent(ConversableAgent):
        ...
        def __init__(self, name: str, strict_verification: bool = True, **kwargs):
            ...
            self.register_reply([Agent, None], self._verify_sender_identity, position=0)
            ...
            if not agent_uri:
                if self.strict_verification:
                    ...
                    raise VerificationError(
                        f"Agent '{sender_name}' did not provide an agent_uri."
                    )
                return False, None

            logger.info(
                f"AutoGen agent '{self.name}' verifying incoming sender '{agent_uri}'"
            )
            try:
                ...
                if not result.valid:
                    ...
                    last_message["content"] = f"[WARNING: SENDER NOT VERIFIED] " + str(
