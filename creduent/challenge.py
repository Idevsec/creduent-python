...
def create_proof(
    ...
    if response.status_code != 200:
        ...
        try:
            ...
            pass
        raise CreduentError(
            f"Failed to fetch challenge: HTTP {response.status_code} - {err_msg}"
        )
    ...
    payload = {"agent_id": agent_id, "nonce": nonce, "signature": signature_b64}
    ...
    if verify_response.status_code != 200:
        ...
        try:
            ...
            pass
        raise CreduentError(
            f"Challenge verification failed: HTTP {verify_response.status_code} - {err_msg}"
        )
    ...
    try:
        ...
        raise CreduentError(
            f"Registry returned invalid JSON for verification response: {e}"
        )
...
def verify_proof(
    ...
    if norm_token_agent_id.startswith("agent:/") and not norm_token_agent_id.startswith(
        "agent://"
    ):
